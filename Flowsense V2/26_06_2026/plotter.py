"""
Flowsense V2 plotter — dati da Test X dati.csv, eventi da Test X orari e percentuali.xlsx.
...
st.set_page_config(page_title="OilSense Plotter — Flowsense V2", layout="wide")
st.title("OilSense Data Plotter — Flowsense V2")
...
def test_paths(base_dir: Path, test_num: int) -> dict[str, Path]:
    return {
        "dati": base_dir / f"Test {test_num} dati.csv",
        "eventi": base_dir / f"Test {test_num} orari e percentuali.xlsx",
        "descrizione": base_dir / f"test {test_num} descrizione.txt",
    }
...
if not available_tests:
    st.error("Nessun test trovato in Flowsense V2 (attesi file 'Test N dati.csv').")
    st.stop()
...
paths = test_paths(WORK_DIR, selected_test)
missing = [name for name, path in paths.items() if name != "descrizione" and not path.exists()]
if missing:
    st.error(f"File mancanti per Test {selected_test}: {', '.join(missing)}")
    st.stop()
...
if df_raw.empty:
    st.warning("Nessun dato nel file CSV selezionato.")
    st.stop()
...
fig.update_layout(
    ...
    title_text=(
        f"Test {selected_test} — valori mediati su {MOVING_AVERAGE_WINDOW} punti "
